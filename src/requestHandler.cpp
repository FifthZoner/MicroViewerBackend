...
inline std::unordered_map<std::string, void (*)(const Http::Request&, Http::ResponseWriter&)> endpoints;
...
void endpoint_alive(const Http::Request& request, Http::ResponseWriter& response) {
    response.send(Http::Code::Ok, "MicroViewer is alive!\n");
    std::cout << "Sent alive message\n";
}

void endpoint_favicon(const Http::Request& request, Http::ResponseWriter& response) {
    response.send(Http::Code::No_Content, "No favicon for backend!\n");
}

void endpoint_getCategories(const Http::Request& request, Http::ResponseWriter& response) {
...
void prepareEndpoints() {
    endpoints.emplace("/"           , &endpoint_alive        );
    endpoints.emplace("/favicon.ico", &endpoint_favicon      );
    endpoints.emplace("/categories" , &endpoint_getCategories);
}